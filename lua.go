...
func NewLuaMigration(id string, upRequest string, downRequest string) *Migration {
	return &Migration{
		...
		Migrate: func(tt pool.Pooler, ctx context.Context, opts *Options) error {
			_, err := tt.Do(tarantool.NewEvalRequest(upRequest).Context(ctx), opts.WriteMode).Get()
			if err != nil {
				return err
			}
			return nil
		...
		Rollback: func(tt pool.Pooler, ctx context.Context, opts *Options) error {
			_, err := tt.Do(tarantool.NewEvalRequest(downRequest).Context(ctx), opts.WriteMode).Get()
			if err != nil {
				return err
			}
			return nil
